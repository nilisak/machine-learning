...
def plot_mean(l1, l2, l3, log_x=True, log_y=True):

    # Plot each array
    plt.figure(figsize=(12, 6))  # Set the figure size for better visibility
    plt.plot(range(1, len(l1) + 1), l1, "k--", label="MNIST")
    plt.plot(range(1, len(l2) + 1), l2, "b--", label="KMNIST")
    plt.plot(range(1, len(l3) + 1), l3, "r--", label="FashionMNIST")
    ...
    plt.plot(range(1, len(l2) + 1), l2 / max(l2), "b--", label="KMNIST")
    plt.plot(range(1, len(l3) + 1), l3 / max(l3), "r--", label="FashionMNIST")
...
def main():
    ...
    kmnist_S = np.load(filenames[1])
    fmnist_S = np.load(filenames[2])

    plot_singular_values_by_label(mnist_S, log_x=False, log_y=True, data_set="MNIST")
    plot_singular_values_by_label(kmnist_S, log_x=False, log_y=True, data_set="KMNIST")
    plot_singular_values_by_label(fmnist_S, log_x=False, log_y=True, data_set="FashionMNIST")

    average_M = np.mean(mnist_S, axis=(0, 2))
    average_K = np.mean(kmnist_S, axis=(0, 2))
    average_F = np.mean(fmnist_S, axis=(0, 2))

    plot_mean(average_M, average_K, average_F, log_x=True, log_y=True)
