...
class MNISTDataModule(L.LightningDataModule):
    ...
    def __init__(self, data_root, batch_size=16, pred_batch_size=10):
        super().__init__()
...
class KMNISTDataModule(L.LightningDataModule):
    ...
    def prepare_data(self):
        KMNIST(self.data_root, train=True, download=True)
        KMNIST(self.data_root, train=False, download=True)

    def setup(self, stage):

        self.val_dataset = KMNIST(
            self.data_root, train=False, download=True, transform=transforms.ToTensor()
        )

        self.train_dataset = KMNIST(
            self.data_root, train=True, download=True, transform=transforms.ToTensor()
        )
...
class FashionMNISTDataModule(L.LightningDataModule):
    ...
    def prepare_data(self):
        FashionMNIST(self.data_root, train=True, download=True)
        FashionMNIST(self.data_root, train=False, download=True)
