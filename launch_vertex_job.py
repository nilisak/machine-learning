...
def upload_blob(source_file_name, destination_blob_name):
    ...
    destination_file_name = "gs://" + str(
        PurePosixPath(BUCKET.split("gs://")[1], destination_blob_name)
    )
...
def launch_script_job(args):
    ...
    log_path = PurePosixPath(
        BUCKET.replace("gs://", "/gcs/"), f"{args.name}_{timestamp}", "log.txt"
    )
    requirements = [
        "torch==1.13",
        "lightning==2.1.2",
...
def launch_package_job(args):
    ...
    destination_blob_name = PurePosixPath(
        "custom-training-python-package", f"{args.name}", f"{timestamp}/{source_name}"
    )
